            ...
            {PACKAGES.map((pkg, i) => (
              <motion.div
                ...
                className={`flex flex-col p-10 rounded-[3rem] border transition-all duration-500 relative ${
                  pkg.highlight 
                    ? 'border-brand-primary shadow-2xl shadow-brand-primary/20 scale-105 z-10 bg-white/5 backdrop-blur-xl' 
                    : 'border-white/5 bg-white/5 hover:border-brand-primary/20 hover:shadow-2xl hover:shadow-brand-primary/5'
                }`}
              >
                {pkg.highlight && (
                  <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-brand-primary text-white text-[10px] uppercase font-black tracking-widest px-6 py-2 rounded-full shadow-lg">
                    Most Popular
                  </div>
                )}
                
                <div className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${pkg.color} flex items-center justify-center text-white mb-8 shadow-lg`}>
                  <pkg.icon size={28} />
                </div>
                
                <h3 className="text-2xl font-bold text-white mb-2">{pkg.name}</h3>
                <p className="text-gray-500 text-sm mb-8 leading-relaxed">{pkg.description}</p>
                ...
                <div className="grow space-y-4 mb-10">
                  {pkg.features.map((feature, j) => (
                    <div key={j} className="flex items-center space-x-3">
                      ...
                      <span className="text-gray-400 font-medium text-sm">{feature}</span>
                    </div>
                  ))}
                </div>

                <button className={`w-full py-5 rounded-full font-black transition-all shadow-lg text-sm uppercase tracking-widest ${
                  pkg.highlight 
                    ? 'bg-brand-primary text-white hover:bg-brand-primary/90 shadow-brand-primary/20' 
                    : 'bg-white/5 text-white border border-white/10 hover:border-brand-primary hover:text-brand-primary'
                }`}>
