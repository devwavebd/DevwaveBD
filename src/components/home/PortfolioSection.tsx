...
export default function PortfolioSection() {
  return (
    <section className="py-24 bg-brand-bg" id="portfolio">
      <div className="custom-container">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
          <div className="max-w-2xl">
            <p className="text-sm font-bold text-brand-primary uppercase tracking-widest mb-3">Our Portfolio</p>
            <h2 className="text-4xl lg:text-5xl font-bold text-white mb-6 font-sans">
              Some of Our Recent Work
            </h2>
          </div>
          <Link to="/portfolio" className="inline-flex items-center text-brand-primary font-bold hover:translate-x-2 transition-transform">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
          {PORTFOLIO.slice(0, 4).map((project, index) => (
            <motion.div
              ...
              className="group"
            >
              <div className="relative h-[480px] rounded-[2rem] overflow-hidden border border-white/10 bg-white/5 shadow-2xl flex flex-col">
                {/* Browser Top Bar */}
                <div className="flex items-center justify-between px-6 py-4 bg-white/5 border-b border-white/5 relative z-20">
                  ...
                  <div className="flex-1 mx-6">
                    <div className="w-full max-w-md mx-auto h-7 rounded-lg bg-white/5 border border-white/5 flex items-center justify-center">
                      <span className="text-[10px] text-gray-500 font-mono tracking-tight uppercase">devwavebd.com/{project.title.toLowerCase().replace(/\s+/g, '-')}</span>
                    </div>
                  </div>
                  <ExternalLink size={16} className="text-gray-500 group-hover:text-brand-primary transition-colors cursor-pointer" />
                </div>
